---
name: pixel-art
description: Build detailed, animated pixel-art scenes that run in the browser (single HTML file, canvas, no dependencies) with a cinematic finish — rain, fire, glowing windows, fog, bloom, lightning, water reflections, particles. Use this skill whenever the user wants pixel art, a retro/8-bit/16-bit look, a cozy or atmospheric animated scene, a "lo-fi" background, a Stardew/Celeste/Terraria-like vignette, an animated wallpaper, or asks to reproduce a pixel-art image, GIF or screen recording "with the same level of detail and animation" — including in French ("pixel art", "scène animée", "reproduis la même chose", "même niveau de détail"). Also use it to improve or add effects to an existing canvas pixel-art scene.
---
...
Run `node scripts/screenshot.js <file.html> <out.png> [waitMs]` once (uses the preinstalled
